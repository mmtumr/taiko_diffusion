...
def load_rating_rows(path: Path) -> list[RatingRow]:
    ...
    for row_index, values in enumerate(
        ...
        if not title:
            continue
        title_without_ura, is_ura = strip_ura_marker(title)
    ...
    def finish_course() -> None:
        ...
        courses.append(
            EseCourse(
                ...
                roll_start_count=roll_start_count,
...
def build_matches(rating_rows: list[RatingRow], courses: list[EseCourse]) -> tuple[list[dict], list[dict], list[dict]]:
            ...
            ambiguous.append(
                {
                    **asdict(rating),
                    ...
                    "candidates": json.dumps([asdict(c) for c in candidates[:10]], ensure_ascii=False),
                }
            )
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare Taiko chart encoder dataset manifests.")
    parser.add_argument("--ese-root", type=Path, default=Path("ESE-master/ese"))
    parser.add_argument("--rating-xlsx", type=Path, default=Path("rating计算工具_11.25.xlsx"))
    parser.add_argument("--output-dir", type=Path, default=Path("taiko_encoder_out"))
    ...
    summary = {
        ...
        "strict_combo_rule": f"combo_delta <= max({args.strict_combo_abs_min}, round(combo * {args.strict_combo_ratio}))",
